...
export default function ComparePage() {
  return (
    <div className="max-w-[1280px] mx-auto px-6 py-10">
      ...
      <div className="mb-8">
        <Link href="/products" className="flex items-center gap-1.5 text-[12px] text-[#86868b] hover:text-black transition-colors mb-4 w-fit">
          <ArrowLeft size={12} /> Back to Browse
        </Link>
        <p className="text-[11px] font-semibold text-[#86868b] tracking-[0.1em] uppercase mb-1.5">Side-by-Side Analysis</p>
        <h1 className="text-[36px] font-medium text-black tracking-tight">Compare Software</h1>
      </div>

      {/* AI recommendation banner */}
      <div className="flex items-center justify-between bg-[#EBF1FD] border border-[#007AFF]/20 rounded-sm px-5 py-3.5 mb-8">
        <p className="text-[13px] text-black">
          <span className="font-semibold text-[#007AFF]">AI Recommendation:</span> Based on your profile,{' '}
        ...
        <Link href={`/products/${compareProducts[0].slug}`} className="flex items-center gap-1 text-[12px] font-semibold text-[#007AFF] hover:text-[#0051D5] transition-colors shrink-0 ml-4">
      ...
      <div className="border border-black/8 rounded-sm overflow-hidden">
        ...
        <div className="grid border-b border-black/8" style={{ gridTemplateColumns: '200px repeat(3, 1fr)' }}>
          <div className="bg-[#f9fafb] p-4 border-r border-black/8">
            <p className="text-[11px] font-semibold text-[#86868b] uppercase tracking-[0.07em]">Product</p>
          </div>
            ...
            <div key={p.id} className={`p-5 ${i < 2 ? 'border-r border-black/8' : ''} ${i === 0 ? 'bg-[#f0f7ff]' : 'bg-white'}`}>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#86868b]">{p.vendor}</p>
                </div>
              </div>
              {p.fitScore && (
                <div className={`inline-flex text-[10px] font-semibold px-2 py-0.5 rounded-full ${i === 0 ? 'bg-[#dcfce7] text-[#16a34a]' : 'bg-[#f5f5f7] text-[#86868b]'}`}>
                  {p.fitScore}% match
                </div>
              )}
            </div>
          ...
          {compareProducts.map((p, i) => (
            <div key={p.id} className={`p-4 flex items-center ${i < 2 ? 'border-r border-black/8' : ''} ${i === 0 ? 'bg-[#f0f7ff]' : ''}`}>
              <div className="flex items-center gap-1">
                {[1,2,3,4,5].map(s => (
                  <span key={s} className={`text-[14px] ${s <= Math.round(p.rating) ? 'text-[#007AFF]' : 'text-[#e5e5e7]'}`}>★</span>
                ))}
          ...
          {compareProducts.map((p, i) => (
            <div key={p.id} className={`p-4 flex items-center ${i < 2 ? 'border-r border-black/8' : ''} ${i === 0 ? 'bg-[#f0f7ff]' : ''}`}>
              <div>
                ...
                {p.startingPrice > 0 && <p className="text-[10px] text-[#86868b]">AED {Math.round(p.startingPrice * 3.67)}/mo</p>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid border-b border-black/8" style={{ gridTemplateColumns: '200px repeat(3, 1fr)' }}>
          ...
          {compareProducts.map((p, i) => (
            <div key={p.id} className={`p-4 flex items-center ${i < 2 ? 'border-r border-black/8' : ''} ${i === 0 ? 'bg-[#f0f7ff]' : ''}`}>
              <span className="bg-[#f5f5f7] text-[#333333] text-[11px] font-medium px-2.5 py-1 rounded-sm">{p.deployment}</span>
            </div>
          ))}
        </div>
          ...
          <div key={feature} className={`grid ${idx < allFeatures.length - 1 ? 'border-b border-black/8' : ''}`} style={{ gridTemplateColumns: '200px repeat(3, 1fr)' }}>
            ...
            {compareProducts.map((p, i) => {
              ...
              return (
                <div key={p.id} className={`p-4 flex items-center ${i < 2 ? 'border-r border-black/8' : ''} ${i === 0 ? 'bg-[#f0f7ff]' : ''}`}>
                  {has
                    ? <Check size={15} className="text-[#007AFF]" strokeWidth={2.5} />
          ...
          {compareProducts.map((p, i) => (
            <div key={p.id} className={`p-4 ${i < 2 ? 'border-r border-black/8' : ''}`}>
              <Link
                ...
                className={`w-full flex items-center justify-center gap-2 py-2.5 text-[12px] font-medium rounded-sm transition-colors ${i === 0 ? 'bg-[#007AFF] text-white hover:bg-[#0051D5]' : 'bg-white text-black border border-black/10 hover:bg-[#f5f5f7]'}`}
              >
