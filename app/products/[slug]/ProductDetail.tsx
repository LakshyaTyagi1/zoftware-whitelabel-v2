...
function StarFill({ rating, size = 13 }: { rating: number; size?: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star key={i} size={size} fill={i <= Math.round(rating) ? '#007AFF' : 'none'} className={i <= Math.round(rating) ? 'text-[#007AFF]' : 'text-[#e5e5e7]'} />
      ))}
    </div>
  );
}
    ...
    <div className="bg-white min-h-screen pb-20 md:pb-24">
      ...
      <div className="border-b border-black/8 bg-[#f9fafb]">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 h-10 flex items-center gap-1.5 overflow-x-auto">
          <Link href="/products" className="flex items-center gap-1 text-[12px] text-[#86868b] hover:text-black transition-colors shrink-0">
            <ArrowLeft size={11} /> All Software
          </Link>
          <ChevronRight size={10} className="text-[#c7c7cc] shrink-0" />
          <Link href={`/products?category=${product.categorySlug}`} className="text-[12px] text-[#86868b] hover:text-black transition-colors shrink-0">{product.category}</Link>
          <ChevronRight size={10} className="text-[#c7c7cc] shrink-0" />
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {product.isNew && <span className="bg-[#dbeafe] text-[#007AFF] text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0">New</span>}
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[12px] text-[#86868b]">({product.reviews.toLocaleString()} reviews)</span>
              {product.fitScore && (
          ...
          <div className="hidden md:flex items-center gap-2 shrink-0">
            <Link href={`/checkout?slug=${product.slug}&plan=${product.plans[0]?.name}&price=${product.plans[0]?.price}`}
              className="flex items-center gap-2 bg-[#007AFF] text-white px-5 py-2.5 text-[13px] font-medium rounded-sm hover:bg-[#0051D5] transition-colors">
              Buy Now <ArrowRight size={13} strokeWidth={2} />
            </Link>
            <Link href="/sign-in" className="flex items-center gap-2 border border-black/10 bg-white text-black px-4 py-2.5 text-[13px] font-medium rounded-sm hover:bg-[#f5f5f7] transition-colors">
        ...
        <div className="flex gap-8 items-start">
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="space-y-6">
                ...
                <div className="border border-black/8 rounded-sm p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setActiveTab('Features')} className="text-[11px] font-semibold text-[#007AFF] hover:text-[#0051D5] transition-colors">
                      See all features →
                    </button>
                  </div>
                ...
                <div className="border border-black/8 rounded-sm p-5">
                  ...
                  <button onClick={() => setActiveTab('Reviews')} className="mt-4 text-[12px] font-semibold text-[#007AFF] hover:text-[#0051D5] transition-colors">
              ...
              <div className="space-y-6">
                <div className="border border-black/8 rounded-sm p-5">
                  <div className="flex gap-6 mb-6 flex-wrap">
                    <div>
                      <p className="text-[32px] font-medium text-[#007AFF] leading-none">{product.features.length}</p>
                      <p className="text-[12px] text-[#86868b] mt-1">Features</p>
                    </div>
                    <div>
                      <p className="text-[32px] font-medium text-[#007AFF] leading-none">{product.ratings.features.toFixed(1)}</p>
                      <p className="text-[12px] text-[#86868b] mt-1">Feature Rating</p>
                    </div>
                    <div>
                      <p className="text-[32px] font-medium text-[#007AFF] leading-none">{product.reviews.toLocaleString()}</p>
                      <p className="text-[12px] text-[#86868b] mt-1">Verified Reviews</p>
                    </div>
                  </div>
                  <p className="text-[11px] font-semibold text-[#86868b] uppercase tracking-[0.07em] mb-4">All Features</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            {activeTab === 'Pricing' && (
              <div className="space-y-6">
                {/* Pricing Overview */}
                <div className="bg-[#f9fafb] border border-black/8 rounded-sm p-5">
                  <p className="text-[11px] font-semibold text-[#86868b] uppercase tracking-[0.07em] mb-3">Pricing Overview</p>
                  <p className="text-[14px] text-[#333333] leading-[1.75]">{product.pricingOverview}</p>
                </div>

                {/* Plans */}
                <div>
                  <p className="text-[11px] font-semibold text-[#86868b] uppercase tracking-[0.07em] mb-4">{product.name} Pricing Plans</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {product.plans.map((plan, i) => (
                      <div key={plan.name} className={`border rounded-sm p-5 flex flex-col ${i === 1 ? 'border-[#007AFF] bg-[#EBF1FD]' : 'border-black/8 bg-white'}`}>
                        {i === 1 && <span className="inline-block bg-[#007AFF] text-white text-[9px] font-semibold px-2 py-0.5 rounded-full mb-2 w-fit">Most Popular</span>}
                        ...
                        <div className="mb-1">
                          ...
                          {plan.price > 0 && <span className="text-[12px] text-[#86868b] ml-1">/{plan.billing}</span>}
                        </div>
                        {plan.price > 0 && (
                          <p className="text-[11px] text-[#86868b] mb-4">≈ AED {Math.round(plan.price * aedRate)}/mo</p>
                        )}
                        <div className="space-y-2 mb-5 flex-1">
                          {plan.features.map(f => (
                            <div key={f} className="flex items-start gap-2">
                              <Check size={11} className="text-[#007AFF] mt-0.5 shrink-0" strokeWidth={2.5} />
                        ...
                        <Link
                          ...
                          className={`w-full flex items-center justify-center gap-2 py-2.5 text-[13px] font-medium rounded-sm transition-colors ${i === 1 ? 'bg-[#007AFF] text-white hover:bg-[#0051D5]' : 'bg-white text-black border border-black/10 hover:bg-[#f5f5f7]'}`}
                        >
                ...
                <div className="border border-black/8 rounded-sm p-5">
                  <div className="flex items-start gap-6 flex-wrap">
                    <div className="text-center">
                      ...
                      <p className="text-[11px] text-[#86868b] mt-1">{product.reviews.toLocaleString()} reviews</p>
                    </div>
                    ...
                    <div key={review.id} className="border border-black/8 rounded-sm p-5">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <div>
                          ...
                          <p className="text-[11px] text-[#86868b]">{review.role}, {review.company}</p>
                        </div>
                        <div className="text-right shrink-0">
                          <StarFill rating={review.rating} />
                          <p className="text-[11px] text-[#86868b] mt-0.5">{review.date}</p>
                        </div>
                      </div>
            ...
            {activeTab === 'Alternatives' && (
              <div className="space-y-4">
                <p className="text-[11px] font-semibold text-[#86868b] uppercase tracking-[0.07em] mb-4">Alternatives to {product.name}</p>
                {alternatives.map(alt => (
                  <Link key={alt.id} href={`/products/${alt.slug}`}
                    ...
                    <div className="w-12 h-12 rounded-sm bg-[#f5f5f7] border border-black/8 flex items-center justify-center shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span className="bg-[#f5f5f7] text-[#86868b] text-[10px] px-2 py-0.5 rounded-sm">{alt.category}</span>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[11px] text-[#86868b]">{alt.rating} ({alt.reviews.toLocaleString()})</span>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-[13px] font-semibold text-black">{alt.startingPrice === 0 ? 'Free' : `$${alt.startingPrice}`}</p>
                      {alt.startingPrice > 0 && <p className="text-[10px] text-[#86868b]">/{alt.billingCycle}</p>}
                      <span className="text-[11px] text-[#007AFF] font-medium mt-2 flex items-center gap-0.5 justify-end">View <ArrowRight size={10} /></span>
                    </div>
                  </Link>
                ))}
                <Link href="/compare" className="flex items-center justify-center gap-2 border border-black/10 rounded-sm py-3 text-[13px] font-medium text-[#007AFF] hover:bg-[#f5f5f7] transition-colors">
          ...
          <div className="w-60 xl:w-64 shrink-0 hidden lg:block">
            <div className="sticky top-28 space-y-3">
              ...
              {/* Buy card */}
              <div className="border border-black/8 rounded-sm p-5">
                <p className="text-[10px] font-semibold text-[#86868b] uppercase tracking-[0.07em] mb-2">Starting from</p>
                <p className="text-[30px] font-medium text-black tracking-tight leading-none mb-0.5">{priceDisplay}</p>
                {product.startingPrice > 0 && (
                  <>
                    <p className="text-[12px] text-[#86868b] mb-0.5">per {product.billingCycle}</p>
                    <p className="text-[11px] text-[#86868b] mb-4">≈ AED {Math.round(product.startingPrice * aedRate)}/mo</p>
                  </>
                )}
                <div className="space-y-2 mt-4">
                  <Link href={`/checkout?slug=${product.slug}&plan=${product.plans[0]?.name}&price=${product.plans[0]?.price}`}
                    className="w-full flex items-center justify-center gap-2 bg-[#007AFF] text-white py-2.5 text-[13px] font-medium rounded-sm hover:bg-[#0051D5] transition-colors">
                    Buy Now <ArrowRight size={13} strokeWidth={2} />
                  </Link>
                  <Link href="/sign-in" className="w-full flex items-center justify-center gap-2 bg-white text-black border border-black/10 py-2.5 text-[13px] font-medium rounded-sm hover:bg-[#f5f5f7] transition-colors">
                    <Calendar size={13} strokeWidth={1.5} /> Schedule Demo
                  </Link>
                  <a href={product.website} target="_blank" rel="noopener noreferrer"
                    className="w-full flex items-center justify-center gap-1 text-[11px] text-[#86868b] hover:text-black transition-colors py-1.5">
              ...
              <div className="bg-[#f9fafb] border border-black/8 rounded-sm p-4">
                ...
                <p className="text-[11px] text-[#86868b] mb-3">See how {product.name} stacks up against similar tools.</p>
                <Link href="/compare" className="flex items-center gap-1 text-[12px] font-semibold text-[#007AFF] hover:text-[#0051D5] transition-colors">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-black/8 px-4 py-3 flex items-center gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-[#86868b]">{priceDisplay}{product.startingPrice > 0 ? `/${product.billingCycle}` : ''}</p>
        </div>
        <Link href="/sign-in" className="flex items-center gap-1.5 border border-black/10 text-black px-3 py-2 text-[12px] font-medium rounded-sm hover:bg-[#f5f5f7] transition-colors shrink-0">
          <Calendar size={12} /> Demo
        </Link>
        <Link href={`/checkout?slug=${product.slug}&plan=${product.plans[0]?.name}&price=${product.plans[0]?.price}`}
          className="flex items-center gap-1.5 bg-[#007AFF] text-white px-4 py-2 text-[12px] font-semibold rounded-sm hover:bg-[#0051D5] transition-colors shrink-0">
      ...
      <div className="hidden lg:block fixed bottom-0 left-0 right-0 z-40 bg-black border-t border-white/10">
        <div className="max-w-[1280px] mx-auto px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href={`/checkout?slug=${product.slug}&plan=${product.plans[0]?.name}&price=${product.plans[0]?.price}`}
              className="flex items-center gap-2 bg-[#007AFF] text-white px-5 py-2 text-[13px] font-semibold rounded-sm hover:bg-[#0051D5] transition-colors">
              Buy Now <ArrowRight size={13} strokeWidth={2} />
            </Link>
          </div>
        </div>
